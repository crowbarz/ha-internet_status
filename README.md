...
| `scan_interval` | [scan_interval](https://www.home-assistant.io/docs/configuration/platform_options/) | 1 min | default polling frequency for all links. `scan_interval` should be set to a value that is greater than `timeout` x `retries` to prevent polling timeouts exceeding the polling interval.
...
| `links` | object | **Required** | a set of objects representing each link, [see below](#links-object).

### `links` object

Similar to how scripts and automations are configured, each link is defined with the `link_id` as the object key. The following properties are supported:

| Name | Type | Default | Description
| ---- | ---- | ------- | -----------
| `name` | string | `Link `_n_` Status` | friendly name for the link
